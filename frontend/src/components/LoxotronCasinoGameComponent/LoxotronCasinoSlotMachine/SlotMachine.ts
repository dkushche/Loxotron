...
import { Screen, SvgDrawableObject, FillObject } from "./DrawableObject";
...
class Tile extends SvgDrawableObject {
    ...
    move(deltaY: number) {
        ...
        const teleport = (Tile.amount - 1) * (Tile.side + Tile.space) + Tile.space;
        const leftBottom = this.y + Tile.side;

        if(leftBottom >= teleport) {
            this.y = 0 - Tile.side + (teleport - leftBottom);
        }
    }
...
class Reel {
    ...
    constructor(x: number, speed: number) {
        ...
        this.speed = Math.random() * 10 * (Tile.side + Tile.space);
        // this.move();
        this.speed = speed;
    }
...
class SlotMachine {
    private reels: Reel[];
    private topBumper: FillObject;
    private bottomBumper: FillObject;
    private frame: SvgDrawableObject;

    constructor(canvas: HTMLCanvasElement) {
        this.reels = [];
        this.topBumper = new FillObject(0);
        this.bottomBumper = new FillObject(612);
